...
def upper_bound_heuristic(data, runs, M,
    ...
    while iterations < runs:
        ...
        prob1 = cp.Problem(cp.Minimize(-cp.log_det(H1)
                                       -cp.log_det(H2)),
                           cons)
...
def branch_and_bound(data, runs, M, tol):
    """
    e
    """
    dim, data_size = data.shape
    ...
    prob = cp.Problem(
        cp.Minimize(-cp.log_det(H1)-cp.log_det(H2)),
                           cons)
...
if __name__ == '__main__':
    ...
    start = time.time()
    H_1, H_2, solut_y, up = branch_and_bound(new_X, 
                                      10000, 2000, 1e-6)
